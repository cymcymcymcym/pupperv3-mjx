...
def reward_force_following(
    pipeline_state: base.State,
    leash_attachment_point: jax.Array,
    leash_target_pos: jax.Array,
    leash_slack: float,
    ...
    """
    diff = leash_target_pos - leash_attachment_point
    distance = jp.linalg.norm(diff)
    stretch = jp.maximum(0.0, distance - leash_slack)
    sigma = jp.maximum(1e-6, tracking_sigma)
    reward = jp.exp(-(stretch**2) / (sigma**2))
